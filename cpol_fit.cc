...
This code describes the effects of birefringence at the South Pole AND uses adjustable rotations of COF for use of the global fit
...
int psiModel(int argc, char** argv, const Double_t* par_fit, Double_t* &fit_depths, Double_t* &psi_median_model, int Station_Fit, Long64_t num_entries_fit) {  
    ...
    const int minstation=Station_Fit;
    const int maxstation=Station_Fit;
    ...
    for (int i=minstation;i<=maxstation;i++) {
        ...
        //ASG
        double depth_rotation0 = DEPTH; // Depth at this step

        // Create a RotationParameters object
        RotationParameters rotationParams0(par_fit, depth_rotation0);
        ...
        n_e1,n_e2,p_e1,p_e2,rotationParams0);
    }
    ...
    for (int i=minstation;i<=maxstation;i++) {
        ...
        for (int idepth=0;idepth<vdepth[i].size();idepth++) {
            ...
            if (getresults[6]!=-1000 || getresults[8]!=-1000 || getresults[7]!=0) {
                ...
                // ASG: Add depth dependence to the rotation parameters
                double depth_rotation1 = zs[0]; // Depth at this step

                //  cout << "zs: " << zs[0] << endl;
                // Create a RotationParameters object
                RotationParameters rotationParams1(par_fit, depth_rotation1);
                ...
                double deltan_alongpath=getDeltaN(BIAXIAL,nvec_thisstep,rhat_thisstep,angle_iceflow,n_e1,n_e2,p_e1,p_e2,rotationParams1);
                ...
                for (int istep=UZAIRSTEP;istep<res.size();istep+=UZAIRSTEP) {
                    ...
                    if (istep>0) {
                        ...
                        // ASG: Add depth dependence to the rotation parameters
                        double depth_rotation2 = zs[istep]; // Depth at this step

                        // Create a RotationParameters object
                        RotationParameters rotationParams2(par_fit, depth_rotation2);
                        ...
                        deltan_alongpath=getDeltaN(BIAXIAL,nvec_thisstep,rhat_thisstep,angle_iceflow,n_e1,n_e2,p_e1,p_e2,rotationParams2);
    ...
    */
    ...
    //ASG: Stop the function at this point
    return 0;      
    TGraph *g1[NSTATIONS];
    ...
    for (int i=minstation;i<=maxstation;i++) {
        ...
        for (int j=0;j<NDISTANCES_BIGPIC-(jmin[i]+1);j++) {
            ...
            if (getresults[6]!=-1000 || getresults[8]!=-1000) {
                ...
                //ASG
			 	double depth_rotation3 = zs[istep]; // Depth at this step

                // Create a RotationParameters object
                RotationParameters rotationParams3(par_fit, depth_rotation3);
                    ...
                    if (istep>0) {
                        ...
                        double deltan_alongpath=getDeltaN(BIAXIAL,nvec_thisstep,rhat_thisstep,angle_iceflow,n_e1,n_e2,p_e1,p_e2,rotationParams3);
