...
//ASG ROTATION PARAMETERS DPETH DEPENDENT
struct RotationParameters {
    double gamma; // Rotation along x-axis
    double theta; // Rotation along y-axis
    double phi;   // Rotation along z-axis

    RotationParameters(const Double_t* par_fit, double depth_rotation) {
        // Compute depth-dependent gamma, theta, and phi
        gamma = 0; /*(par_fit[0] + par_fit[1] * (depth_rotation - 1000) +
                 par_fit[2] * (depth_rotation - 1000) * (depth_rotation - 1000) 
                 // + par_fit[3] * (depth_rotation - 1000) * (depth_rotation - 1000) * (depth_rotation - 1000) 
                 //+ par_fit[4] * (depth_rotation - 1000) * (depth_rotation- 1000) * (depth_rotation - 1000) * (depth_rotation - 1000)
								)* (3.1415926 / 180.0);*/

        theta = 0; /*(par_fit[5] + par_fit[6] * (depth_rotation - 1000) +
                 par_fit[7] * (depth_rotation - 1000) * (depth_rotation - 1000) 
                 // + par_fit[8] * (depth_rotation - 1000) * (depth_rotation - 1000) * (depth_rotation - 1000) 
                 // + par_fit[9] * (depth_rotation - 1000) * (depth_rotation- 1000) * (depth_rotation - 1000) * (depth_rotation - 1000)
								)*(3.1415926 / 180.0);*/

				// Let's divide by a thousand
        phi = 0; /*(par_fit[10] + par_fit[11] * (depth_rotation - 1000) / 1000 +
               par_fit[12] * (depth_rotation - 1000) / 1000 * (depth_rotation - 1000) / 1000
                + par_fit[13] * (depth_rotation - 1000) / 1000 * (depth_rotation - 1000) / 1000 * (depth_rotation - 1000) / 1000
                + par_fit[14] * (depth_rotation - 1000) / 1000 * (depth_rotation- 1000) / 1000 * (depth_rotation - 1000) / 1000 * (depth_rotation - 1000) / 1000
               )*(3.1415926 / 180.0); */
				// MACHTAY LOOK HERE
				// Printing to check that these have actual values
...
double getDeltaN(int BIAXIAL,vector<double> nvec,TVector3 rhat,double angle_iceflow,
                 double &n_e1, double &n_e2,TVector3 &p_e1,TVector3 &p_e2, const RotationParameters& rotationParams); // finds indices of refraction of two rays and unit vectors in the direction of the eigenvectors of D
